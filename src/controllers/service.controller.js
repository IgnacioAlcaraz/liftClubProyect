...
const createService = async (req, res) => {
  try {
    const user = req.user;
    if (user.role !== "coach") {
      return res.status(403).json({ message: "Solo los coaches pueden crear servicios" });
    }

    const savedService = await serviceService.createService(req.body, user.userId);
...
const updateServiceById = async (req, res) => {
  try {
    ...
    const updatedService = await serviceService.updateServiceById(id, req.body, user.userId);
...
const deleteServiceById = async (req, res) => {
  try {
    ...
    if (error.message.includes("No tienes permiso") || error.message.includes("no tiene un coach")) {
      return res.status(403).json({ message: error.message });
    }
...
const getServiceByCoachId = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error al obtener los servicios del coach", error: error.message });
  }
};
